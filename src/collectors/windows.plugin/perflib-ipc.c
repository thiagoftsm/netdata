...
static bool do_ipc(PERF_DATA_BLOCK *pDataBlock, int update_every) {
    PERF_OBJECT_TYPE *pObjectType = perflibFindObjectTypeByName(pDataBlock, "Objects");
    if (!pObjectType)
        return false;

    static COUNTER_DATA semathores = { .key = "Semaphores" };
...
int do_PerflibIPC(int update_every, usec_t dt __maybe_unused) {
    ...
    do_ipc(pDataBlock, update_every);
